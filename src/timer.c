...
timer_error_t timer_add (const char *timerName)
{
    // Timer should really be my first call
    double now = dclock ();
    timer_id_t *newTimer = (timer_id_t *) malloc (sizeof (timer_id_t));
    int status = 0;
...
timer_error_t timer_start (const char *timerName)
{
    timer_id_t *currentTimer = timer_get_timer_object (timerName);
...
timer_error_t timer_stop (const char *timerName)
{
    const double now = dclock ();
    timer_id_t *currentTimer = timer_get_timer_object (timerName);
...
timer_error_t timer_reset (const char *timerName)
{
    timer_id_t *currentTimer = timer_get_timer_object (timerName);
...
double timer_get_current_interval (const char *timerName)
{
...
double timer_get_last_interval (const char *timerName)
{
    timer_id_t *currentTimer = timer_get_timer_object (timerName);
...
double timer_get_total_interval (const char *timerName)
{
    timer_id_t *currentTimer = timer_get_timer_object (timerName);
...
void timer_print_timers_short ()
{
    for (_timerIter = kh_begin (_timerTable); _timerIter != kh_end (_timerTable); _timerIter ++) {
        timer_id_t *currentTimer = (timer_id_t *) kh_val(_timerTable, _timerIter);
    ...
    for (_timerIter = kh_begin (_timerTable); _timerIter != kh_end (_timerTable); _timerIter ++) {
        ...
        if (kh_exist (_timerTable, _timerIter)) {
            printf ("[%s] Current = %lf, Last recorded = %lf, Total recorded = %lf, Current State = %s\n",
                                        currentTimer->name,
                                        timer_get_current_interval (currentTimer->name),
...
timer_error_t timer_print_timer (const char *timerName)
{
    timer_id_t *currentTimer = timer_get_timer_object (timerName);
...
timer_id_t * timer_get_timer_object (const char *timerName)
{
    int status = 0;
    ...
    TIMER_STATUS = TIMER_SUCCESS;
    return (timer_id_t *) (kh_value (_timerTable, _timerIter));
}
...
{
    uint32_t numTimers = timer_get_num_timers ();
...
int timer_comp(const void *t1, const void *t2) {
    const double st1 = ((const timer_id_t *)t1)->firstStartClock;
    const double st2 = ((const timer_id_t *)t2)->firstStartClock;
...
{
    uint32_t numTimers = timer_get_num_timers ();
    timer_id_t *sortedTimers = calloc(numTimers, sizeof(timer_id_t));
    timer_result_t *results = calloc(numTimers, sizeof(timer_result_t));

    int timerIdx = 0;
    for (_timerIter = kh_begin (_timerTable); _timerIter != kh_end(_timerTable); _timerIter ++) {
        if (kh_exist (_timerTable, _timerIter)) {
            const timer_id_t *curTimer = (const timer_id_t *)kh_val(_timerTable, _timerIter);
            sortedTimers[timerIdx++] = *curTimer;
            //printf ("%s\n", kh_key (_timerTable, _timerIter));
        }
    }

    qsort(sortedTimers, timerIdx, sizeof(timer_id_t), timer_comp);

    for (timerIdx = 0; timerIdx < numTimers; timerIdx++) {
        const timer_id_t *curTimer = &sortedTimers[timerIdx];
...
timer_tree_t * timer_get_results_tree()
{
    int i;
    uint32_t numTimers = timer_get_num_timers ();
    timer_result_t *sorted = timer_get_results_sorted();

    timer_tree_t *tree = (timer_tree_t*)calloc(numTimers, sizeof(timer_tree_t));
    ...
    timer_tree_t *curParent = NULL;
    timer_id_t *curParentTimer = NULL;
    for (i = 0; i < numTimers; i++) {
        timer_result_t *curResult = &sorted[i];
        timer_id_t *curTimer = timer_get_timer_object(curResult->name);

        timer_tree_t *curTimerNode = &tree[i];
        ...
        while (curParent != NULL && curParentTimer->lastStopClock < curTimer->lastStopClock) {
            ...
            curParentTimer = curParent != NULL ? timer_get_timer_object(curParent->name) : NULL;
        }
...
void timer_print_results_tree(int excludeChildTime) {
    int i, j;
    int numTimers = timer_get_num_timers();
    timer_tree_t *tree = timer_get_results_tree();

    char *hasNextSiblings = (char*)calloc(numTimers, sizeof(char));
    for (i = 0; i < numTimers; i++) {
        hasNextSiblings[i] = 0;

        timer_tree_t *curNode = &tree[i];
        for (j = i + 1; j < numTimers; j++) {
            timer_tree_t *otherNode = &tree[j];
    ...
    for (i = 0; i < numTimers; ++i) {
        timer_tree_t *curNode = &tree[i];
        timer_tree_t *nextNode = (i < numTimers - 1) ? &tree[i+1] : NULL;
...
void timer_stop_timers ()
{
    for (_timerIter = kh_begin (_timerTable); _timerIter != kh_end(_timerTable); _timerIter ++) {
        if (kh_exist (_timerTable, _timerIter)) {
            timer_stop (kh_key (_timerTable, _timerIter));
        }
    }
...
timer_error_t timer_finalize ()
{
    ...
    for (_timerIter = kh_begin (_timerTable); _timerIter != kh_end(_timerTable); _timerIter ++) {
        if (kh_exist (_timerTable, _timerIter)) {
            timer_id_t *currentTimer = timer_get_timer_object (kh_key (_timerTable, _timerIter));
